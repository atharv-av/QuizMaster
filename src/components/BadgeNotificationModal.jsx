...
function BadgeNotificationModal({ badges, onClaim, onClose }) {
  ...
  const handleNext = () => {
    setActiveIndex((prevIndex) => (prevIndex === badges.length - 1 ? 0 : prevIndex + 1));
  };
      ...
      <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full animate-fadeIn">
        ...
        <div className="relative flex items-center justify-center">
          ...
          <button
            ...
            className="absolute left-0 p-2 bg-gray-800 text-white rounded-full focus:outline-none transform -translate-x-8 hover:bg-gray-700 transition duration-200"
          >
            <svg
          ...
          <div className="space-y-4 overflow-hidden w-full">
            <div className="flex justify-center">
              <div className="w-72 bg-gradient-to-br from-blue-500 to-indigo-600 p-6 rounded-lg shadow-lg">
                <div className="flex items-center space-x-3 mb-4">
                  <span className="text-3xl">{badges[activeIndex]?.icon}</span>
                  <div>
                    ...
                    {badges[activeIndex]?.score && (
                      <div className="text-sm text-gray-300">
                        Score: {badges[activeIndex]?.score}
                      </div>
                    )}
                    {badges[activeIndex]?.streak && (
                      <div className="text-sm text-gray-300">
                        Streak: {badges[activeIndex]?.streak}
                      </div>
                    )}
                  </div>
                </div>
                <button
                  onClick={() => onClaim(badges[activeIndex]?.id)}
                  className="w-full bg-green-500 hover:bg-green-600 text-white px-3 py-2 rounded-md text-sm transition duration-200"
                >
                  Claim
                </button>
              </div>
          ...
          <button
            onClick={handleNext}
            className="absolute right-0 p-2 bg-gray-800 text-white rounded-full focus:outline-none transform translate-x-8 hover:bg-gray-700 transition duration-200"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5"
